...
def control_move_to_me_speed(vx, vy, wz, wheel_radius, robot_lenght, robot_width):
    ...
    vy=input("y方向速度:")
    wz=input("z方向速度:")
    robot_lenght=input("轮轴与底盘中心纵间距:")
    robot_width=input("轮轴与底盘中心纵间距:")
    vx=float(vx)
    vy=float(vy)
    wz=float(wz) 
    robot_lenght=float(robot_lenght)
    robot_width=float(robot_width)
    w1=vx+vy+wz*(robot_lenght+robot_width)
    w2=vx-vy+wz*(robot_lenght+robot_width)
    w3=vx+vy+wz*(robot_lenght+robot_width)
    w4=vx=vy+wz*(robot_lenght+robot_width)
    #找到最大车轮速度， 如果最大速度大于1000rpm， 需要将所有车轮速度等比例缩小， 使得最大速度为1000rpm
    if w1>1000 or w2>1000 or w3>1000 or w4>1000:
        list1=[w1,w2,w3,w4]
        list1/k映射
    #返回计算出的速度
    return w1,w2,w3,w4
...
def calc_chassis_positon(last_x, last_y, last_angle, w1, w2, w3, w4, period, wheel_radius, robot_lenght, robot_width):
    ...
    period=0.02
    while period<0.02:
        angle=wz*(robot_lenght+robot_width)
        x=last_x*math.cos(angle)-last_y*math.sin(angle)
        y=last_x*math.sin(angle)+last_y*math.cos(angle)
